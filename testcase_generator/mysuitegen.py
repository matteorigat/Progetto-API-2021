...
if __name__ == '__main__':
    ...
    n = 2**5
    ig.gen_input_file(os.path.join(root, 'input.1'),
    ...
    n = 2**5
    ig.gen_input_file(os.path.join(root, 'input.2'),
    ...
    n = 2**5
    ig.gen_input_file(os.path.join(root, 'input.3'),
                      d_big, k_big, n, edge_p, w_min, w_max,
    ...
    n = 2**5
    ig.gen_input_file(os.path.join(root, 'input.4'),
    ...
    n = 2**10
    ig.gen_input_file(os.path.join(root, 'input.5'),
    ...
    n = 10
    ig.gen_input_file(os.path.join(root, 'input.6'),
                      d_small, k_big, n, edge_p, w_min, w_max,
                      add_topk=topk_multi(n, topk_p), decreasing=True)
    ...
    topk_p = 0.003
    # Test 1 (Normal)
    print('Generating test 7...', end='', flush=True)
    n = 2**20 + 2**18 + 2**17
    ig.gen_input_file(os.path.join(root, 'input.7'),
                      d_small, k_small, n, edge_p, w_min, w_max,
                      add_topk=ig.topk_endonly(n))

    # Test 2 (Normal Big)
    print(' done.\nGenerating test 8...', end='', flush=True)
    n = 2**8
    ig.gen_input_file(os.path.join(root, 'input.8'),
                      d_big, k_big * 16, n, edge_p, w_min, w_max,
                      add_topk=ig.topk_endonly(n), decreasing=True)

    # Test 3 (Normal Big + TopK)
    print(' done.\nGenerating test 9...', end='', flush=True)
    n = 2**8
    ig.gen_input_file(os.path.join(root, 'input.9'),
                      d_big, k_big * 16, n, edge_p, w_min, w_max,
                      add_topk=topk_multi(n, topk_p), decreasing=True)

    # Test 4 (Dijkstra)
    print(' done.\nGenerating test 10...', end='', flush=True)
    n = 2**7 + 2**6 + 2**5
    ig.gen_input_file(os.path.join(root, 'input.10'),
                      d_big, k_small, n, edge_p, w_min, w_max,
                      add_topk=ig.topk_endonly(n))

    # Test 5 (Insert)
    print(' done.\nGenerating test 11...', end='', flush=True)
    n = 2**20 + 2**18
    ig.gen_input_file(os.path.join(root, 'input.11'),
                      d_small, k_big * 256, n, edge_p, w_min, w_max,
                      add_topk=ig.topk_endonly(n), decreasing=True)

    # Test 6 (Insert + TopK)
    print(' done.\nGenerating test 12...', end='', flush=True)
    n = 2**16 + 2**14 + 2**13
    ig.gen_input_file(os.path.join(root, 'input.12'),
                      d_small, k_big * 256, n, edge_p, w_min, w_max,
                      add_topk=topk_multi(n, topk_p), decreasing=True)
